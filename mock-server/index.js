...
const paymentsMockUrl = pathToFileURL(path.join(projectRoot, 'src', 'data', 'payments.mock.js')).href;
const disputesMockUrl = pathToFileURL(path.join(projectRoot, 'src', 'data', 'disputes.mock.js')).href;
const reviewsMockUrl = pathToFileURL(path.join(projectRoot, 'src', 'data', 'reviews.mock.js')).href;
const usersMockUrl = pathToFileURL(path.join(projectRoot, 'src', 'data', 'users.mock.js')).href;
const metricsMockUrl = pathToFileURL(path.join(projectRoot, 'src', 'data', 'metrics.mock.js')).href;
...
async function fetchPayments({ status = 'ALL', search = '', page = 1, perPage = 15, delayMs = 200 } = {}){
  await sleep(delayMs);
  let data = [...PAYMENTS];
...
const { makeMonthlySales, makeDeviceTraffic, makeWebsiteTraffic, makeLocationTraffic } = await import(metricsMockUrl);
...
app.get('/api/payments', async (req, res) => {
  ...
  const payload = await fetchPayments({ status: status || 'ALL', search: search || '', page: pageNum, perPage: perPageNum });
  res.json(payload);
});
