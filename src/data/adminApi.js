...
import {
  fetchPayments as localFetchPayments,
  fetchPaymentDetail as localFetchPaymentDetail,
} from "./paymentsApi";
...
import {
  ...
  makeWebsiteTraffic,
  makeLocationTraffic,
} from "./metrics.mock.js";

const RAW_API_BASE = import.meta.env.VITE_API_BASE || "";

function normalizeApiBase(raw) {
  if (!raw) return "";
  // If someone provided a port like ":3333" (missing host), assume localhost
  if (raw.startsWith(":")) return `http://localhost${raw}`;
  // If no protocol present, assume http
  if (!/^[a-zA-Z]+:\/\//.test(raw)) return `http://${raw}`;
  return raw;
}
...
if (RAW_API_BASE && RAW_API_BASE !== API_BASE) {
  ...
  console.warn(
    `[adminApi] Normalized VITE_API_BASE from "${RAW_API_BASE}" to "${API_BASE}"`
  );
}
...
export async function fetchDisputes({
  ...
  if (API_BASE) {
    const resp = await apiFetch(
      `/api/disputes?page=${page}&perPage=${perPage}`
    );
